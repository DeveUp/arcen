"""
    @name - UpdateTypeFurnitureService
    @description - Servicio para actualizar un tipo de mueble
    @version - 1.0.0
...
class UpdateTypeFurnitureService(IService):
    ...
    # @override
    # @method - Actualizar un tipo de mueble por su pk
    # @parameter - data - Json con el tipo de mueble a actualizar
    ...
    def execute(self, data:dict):
        try:
            ...
            feign_audit_save_error(
                ...
                self.current_service,
                self.current_operation,
                RESPONSE['type_funiture']['put']['update']['error']['default']
            )
        feign_audit_save(
            self.feign_audit,
            self.current_service,
